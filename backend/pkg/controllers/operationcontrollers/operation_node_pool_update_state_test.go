...
func TestNodePoolUpdateSpecMatchesDesired(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			want:    false,
			wantMsg: "management cluster NodePool replicas is 2, want 3",
		},
		{
		...
		},
		{
			...
			want:    false,
			wantMsg: "management cluster NodePool autoscaling min is 1, want 2",
		},
		{
			...
			want:    false,
			wantMsg: "management cluster NodePool has no autoscaling configuration",
		},
		{
			...
			want:    false,
			wantMsg: "management cluster NodePool still has autoscaling configuration",
		},
		{
			name: "labels match",
			desired: api.HCPOpenShiftClusterNodePoolProperties{
		...
		},
		{
			...
			want:    false,
			wantMsg: "management cluster NodePool nodeLabels do not match desired labels",
		},
		{
			name: "taints match",
			desired: api.HCPOpenShiftClusterNodePoolProperties{
		...
		},
		{
			...
			want:    false,
			wantMsg: "management cluster NodePool has 0 taints, want 1",
		},
		{
		...
		},
		{
			...
			want:    false,
			wantMsg: "management cluster NodePool nodeDrainTimeout is 15m0s, want 30m0s",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, msg := c.specMatchesDesired(tt.desired, tt.observed)
...
func TestNodePoolUpdateStatusMatchesDesired(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			want:     false,
			wantMsg:  "management cluster NodePool status replicas is 2, want 3",
		},
		{
		...
		},
		{
			...
			want:    false,
			wantMsg: "management cluster NodePool autoscaling is not enabled: Disabled: waiting for autoscaler",
		},
		{
			...
			want:     false,
			wantMsg:  "management cluster NodePool status replicas is 1, want at least 2",
		},
		{
			...
			want:     false,
			wantMsg:  "management cluster NodePool status replicas is 6, want at most 5",
		},
		{
			...
			want:    false,
			wantMsg: "management cluster NodePool is not ready: NotReady: machines not ready",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := &operationNodePoolUpdate{}
			got, msg := c.statusMatchesDesired(tt.desired, tt.observed)
			assert.Equal(t, tt.want, got)
			if !tt.want {
				assert.Equal(t, tt.wantMsg, msg)
			}
